...
(defmacro
  ^{:doc (str (:doc (meta #'clojure.core/fn))
...
(defmethod deserialize-val :serfn [_ serialized]
  (let [{:keys [ser-meta ser-env ns source]} (KryoService/deserialize serialized)
        rest-meta (best-effort-map-val (KryoService/deserialize ser-meta)
                                       deserialize)
        env (best-effort-map-val (KryoService/deserialize ser-env)
                                 deserialize)
        source-form (try (read-string source)
                         (catch Exception e
                           (throw
                            (RuntimeException.
                             "Could not deserialize " source))))
        namespace (symbol ns)
        old-ns (-> *ns* str symbol)
        bindings (mapcat (fn [[name val]]
                           [(symbol name) `(*GLOBAL-ENV* ~name)])
                         env)
